            ...
            <div className="flex gap-2">
              {['all', 'active', 'banned'].map(f => (
                <button
                  ...
                  className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all border ${filterStatus === f ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white border-border-light text-text-secondary hover:bg-background-secondary hover:text-text-primary'}`}
                >
              ...
              <tbody className="divide-y divide-border-light">
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-6">
                      <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border ${user.role === 'Premium' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : user.role === 'Corporate' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                        {user.role}
                      </span>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-2">
                        <span className={`h-2.5 w-2.5 rounded-full ${user.status === 'Active' ? 'bg-primary shadow-[0_0_8px_rgba(16,163,16,0.6)] animate-pulse' : user.status === 'Offline' ? 'bg-slate-300' : user.status === 'Banned' ? 'bg-red-500' : 'bg-orange-400'}`} />
