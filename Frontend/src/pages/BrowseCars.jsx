...
import {
  Search, SlidersHorizontal, ChevronDown, Loader2, Users, Fuel, Gauge, AlertCircle
} from 'lucide-react';
import DashboardNavbar from '../components/layout/DashboardNavbar';
import { carService } from '../services/carService';

// --- ROBUST ASSET IMPORTS ---
...
import g63Img from '../assets/mercedesg63amg.png'; // Handled space
...
const getImageForCar = (car) => {
  ...
  if (text.includes('g63') || text.includes('g-wagon')) return g63Img;
  if (text.includes('mercedes')) return mercedesImg;
...
const BrowseCars = () => {
  ...
  const loadCars = async () => {
    try {
      setLoading(true);
      // REMOVED {available: true} so ALL cars show up
      const response = await carService.getAllCars(); 
      
      if (response.success && response.data) {
        setAllCars(response.data);
        setDisplayCars(response.data);
        setBrands(['All', ...new Set(response.data.map(c => c.brand))]);
      } else {
...
// --- CARD COMPONENT (Using Inventory Card Design) ---
